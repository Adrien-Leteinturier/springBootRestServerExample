...
//				.andExpect(jsonPath("$[0].active", is(user.getActive()))).andReturn();
//
...
//	}
	...
	public void testSaveUser() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/user/users").contentType(MediaType.APPLICATION_XML)
				.accept(MediaType.APPLICATION_XML)
				.content("<user><login>Dupont</login><password>password</password><active>1</active></user>"))
				.andExpect(status().isCreated());

		// on s'assure que la méthode de service saveOrUpdateUser(User) a bien été
		// appelée
...
//		mockMvc.perform(get("/user/users/{loginName}", new String("Dupont")).contentType(MediaType.APPLICATION_JSON))
//				.andExpect(status().isFound()).andExpect(jsonPath("$.login", is(user.getLogin())))
//				.andExpect(jsonPath("$.password", is(user.getPassword())))
//				.andExpect(jsonPath("$.active", is(user.getActive())));
//
//		// Résultat: on s'assure que la méthode de service findByLogin(login) a bien été
//		// appelée
...
//	}
	...
	public void testDeleteUser() throws Exception {
		// on exécute le test
		mockMvc.perform(MockMvcRequestBuilders.delete("/user/users/{id}", new Long(1))).andExpect(status().isGone());
	...
	public void testUpdateUser() throws Exception {
		...
		// on exécute la requête
		mockMvc.perform(
				MockMvcRequestBuilders.put("/user/users/{id}", new Long(1)).contentType(MediaType.APPLICATION_XML)
						.accept(MediaType.APPLICATION_XML).content("<user><active>0</active></user>"))
